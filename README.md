...
* A patch to remove the concatenation of multiple cookie headers into one on forward of http2 requests. This is "not a bug, it's a feature" in http2, but we do not want this behavior in our
...
The patch involved deleting the following lines in `fhttp/http2/transport.go`:
    ```go
        ...
        continue
    ```
* A patch to allow non-standard HTTP header names. This was handled in `h2_bundle.go` and `http2/transport.go` (manually, although this was original generated), following calls to `httpguts.ValidHeaderFieldName`.
